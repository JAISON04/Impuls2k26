...
try {
    ...
    // Try using default credentials (for cloud environments)
    admin.initializeApp({
...
async function seedEvents() {
    ...
    for (const event of eventsData.technical) {
    ...
    for (const event of eventsData.online) {
        const docRef = db.collection('events').doc();
        batch.set(docRef, {
            ...event,
            createdAt: admin.firestore.FieldValue.serverTimestamp()
    ...
    for (const workshop of workshopsData) {
        const docRef = db.collection('workshops').doc();
        batch.set(docRef, {
            ...workshop,
            createdAt: admin.firestore.FieldValue.serverTimestamp()
        });
